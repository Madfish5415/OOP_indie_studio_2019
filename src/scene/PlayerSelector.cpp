...
void PlayerSelector::init(ecs::Universe* universe)
{
    ...
    worldManager->addComponent(background,
        ecs::component::Image(driver, scene::playerselector::BACKGROUND, new irr::core::position2d<irr::s32> {0, 0},
            new irr::core::rect<irr::s32> {0, 0, 1920, 1080}));

    ecs::Entity frame = worldManager->createEntity();
    worldManager->addComponent(frame,
        ecs::component::Image(driver, scene::playerselector::FRAME, new irr::core::position2d<irr::s32> {110, 215},
            new irr::core::rect<irr::s32> {0, 0, 1700, 650}, nullptr, irr::video::SColor(255, 255, 255, 255), true));

    ecs::Entity defaultPlayer = worldManager->createEntity();
    worldManager->addComponent(defaultPlayer,
        ecs::component::Image(driver, getUnusedSkin(), new irr::core::position2d<irr::s32> {960 - 150, 245},
            new irr::core::rect<irr::s32> {0, 0, 300, 460}, nullptr, irr::video::SColor(255, 255, 255, 255), true));
...
void PlayerSelector::addPlayer(ecs::Universe* universe)
{
    ...
    ecs::Entity playerSkinButtonLeft = createButton(worldManager, gui, new irr::core::rect<irr::s32>(835, 600, 835 + 40, 600 + 40),
        nullptr, GUI_SELECT_SKIN_P1_LEFT + (PlayerSelector::playerIds.size() - 1) * 2,
    ...
    ecs::Entity playerSkinButtonRight = createButton(worldManager, gui, new irr::core::rect<irr::s32>(1045, 600, 1045 + 40, 600 + 40),
        nullptr, GUI_SELECT_SKIN_P1_RIGHT + (PlayerSelector::playerIds.size() - 1) * 2,
    ...
    for (auto& id : PlayerSelector::playerIds) {
        ...
        right.rect->UpperLeftCorner.X = image.position->X + image.rect->LowerRightCorner.X - 25 - 40;
...
void PlayerSelector::changeSkin(ecs::Universe* universe, irr::s32 id)
{
    switch (id) {
        case BUTTON_ID::GUI_SELECT_SKIN_P1_LEFT: {
            ...
            image.texture = universe->getDevice()->getVideoDriver()->getTexture(prevUnusedSkin.c_str());
        ...
        case BUTTON_ID::GUI_SELECT_SKIN_P1_RIGHT: {
            ...
            image.texture = universe->getDevice()->getVideoDriver()->getTexture(nextUnusedSkin.c_str());
